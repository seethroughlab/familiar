...
`Profile.session_key` is per-profile, the client says which profile it is via `X-Profile-ID`, and the
server scrobbles in the background. The mechanism exists and works.
...
- **Follow-up** — the auth callback returns to the server, which means the app has to poll or be told
  when the exchange completes. Nothing here specifies that handshake, and it is the fiddly part of
  the implementation.
