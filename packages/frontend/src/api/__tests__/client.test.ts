...
describe('spotifyApi', () => {
  ...
  describe('getStatus', () => {
    it('should fetch Spotify connection status', async () => {
      mockApiInstance.get.mockResolvedValue({
        data: { configured: true, connected: true, spotify_user_id: 'user123' },
      })

      const status = await spotifyApi.getStatus()

      expect(mockApiInstance.get).toHaveBeenCalledWith('/spotify/status')
      expect(status.connected).toBe(true)
    })
  })

  describe('sync', () => {
    it('should sync Spotify favorites with long timeout', async () => {
      mockApiInstance.post.mockResolvedValue({
        data: { status: 'running', stats: { fetched: 100 } },
      })

      await spotifyApi.sync(true, false)

      expect(mockApiInstance.post).toHaveBeenCalledWith('/spotify/sync', null, {
        params: { include_top_tracks: true, favorite_matched: false },
