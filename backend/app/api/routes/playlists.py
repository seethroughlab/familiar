...
from app.db.models import ArtistNewRelease, ExternalTrackSource, Playlist, PlaylistTrack, Track
...
class WishlistAddReleaseRequest(BaseModel):
    """Request to add all tracks from a new release to the wishlist."""

    release_id: UUID


class WishlistAddReleaseResponse(BaseModel):
    """Response from adding a release's tracks to the wishlist."""
...
@router.post("/wishlist/add-release", response_model=WishlistAddReleaseResponse)
async def add_release_to_wishlist(
    request: WishlistAddReleaseRequest,
    db: DbSession,
    profile: RequiredProfile,
) -> WishlistAddReleaseResponse:
    """Add all tracks from a new release to the wishlist.

    For Spotify releases, fetches individual tracks from the Spotify API.
    For MusicBrainz releases, falls back to a single album-level entry.
    """
    # Look up the release
    release = await db.get(ArtistNewRelease, request.release_id)
    if not release:
        raise HTTPException(
    ...
    # Non-Spotify releases: fall back to single album-level entry
    if release.source != "spotify":
        external_track = await matcher.create_external_track(
            title=release.release_name,
            artist=release.artist_name,
            album=release.release_name,
            source=ExternalTrackSource.NEW_RELEASE,
            external_data={
        ...
        existing = await db.execute(
            select(PlaylistTrack).where(
                PlaylistTrack.playlist_id == wishlist.id,
                PlaylistTrack.external_track_id == external_track.id,
            )
        )
        if existing.scalar_one_or_none():
            await db.commit()
            return WishlistAddReleaseResponse(
                tracks_added=0,
                tracks_skipped=1,
    ...
    # Spotify release: fetch individual tracks
    spotify_service = SpotifyService()
    client = await spotify_service.get_client(db, profile.id)
    if not client:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Spotify not connected. Connect Spotify in Settings to add release tracks.",
        )

    # Fetch album from Spotify (includes first page of tracks)
    try:
        album = client.album(release.release_id)
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail=f"Failed to fetch album from Spotify: {e}",
        )

    # Collect all tracks, paginating if needed
    spotify_tracks = list(album["tracks"]["items"])
    next_url = album["tracks"].get("next")
    while next_url:
        page = client.next(album["tracks"])
        spotify_tracks.extend(page["items"])
        next_url = page.get("next")
        album["tracks"] = page  # for next iteration's client.next()

    album_artwork = None
    if album.get("images"):
        album_artwork = album["images"][0]["url"]

    album_url = album.get("external_urls", {}).get("spotify")

    # Get existing external_track_ids already in the wishlist for dedup
    existing_result = await db.execute(
        select(PlaylistTrack.external_track_id).where(
            PlaylistTrack.playlist_id == wishlist.id,
            PlaylistTrack.external_track_id.isnot(None),
        )
    )
    existing_ext_ids = {row[0] for row in existing_result.all()}
    ...
    for sp_track in spotify_tracks:
        if not sp_track or not sp_track.get("name"):
            continue

        artists = ", ".join(a["name"] for a in sp_track.get("artists", []))
        duration_s = sp_track.get("duration_ms", 0) / 1000.0
        track_spotify_id = sp_track.get("id")
        track_url = sp_track.get("external_urls", {}).get("spotify")

        external_track = await matcher.create_external_track(
            title=sp_track["name"],
            artist=artists or release.artist_name,
            album=release.release_name,
            source=ExternalTrackSource.NEW_RELEASE,
            spotify_id=track_spotify_id,
            duration_seconds=duration_s if duration_s > 0 else None,
            external_data={
                "artwork_url": album_artwork or release.artwork_url,
                "spotify_url": track_url,
                "album_spotify_url": album_url,
                "source": "spotify",
                "release_type": release.release_type,
            },
            source_playlist_id=wishlist.id,
        ...
        if external_track.id in existing_ext_ids:
            tracks_skipped += 1
            continue
        ...
        existing_ext_ids.add(external_track.id)
        tracks_added += 1
