...
def upgrade() -> None:
    if not _column_exists("track_analysis", "feature_confidence"):
        op.add_column(
            "track_analysis",
            sa.Column("feature_confidence", sa.dialects.postgresql.JSONB, nullable=True),
        )

    if not _column_exists("track_analysis", "local_features"):
        op.add_column(
            "track_analysis",
            sa.Column("local_features", sa.dialects.postgresql.JSONB, nullable=True),
        )

    if not _column_exists("track_analysis", "mood_tags"):
        op.add_column(
            "track_analysis",
    ...
    if not _column_exists("track_analysis", "mood_tags_version"):
        op.add_column(
            "track_analysis",
...
def downgrade() -> None:
    ...
    if _column_exists("track_analysis", "mood_tags_version"):
        op.drop_column("track_analysis", "mood_tags_version")
    if _column_exists("track_analysis", "mood_tags"):
        op.drop_column("track_analysis", "mood_tags")
    if _column_exists("track_analysis", "local_features"):
        op.drop_column("track_analysis", "local_features")
    if _column_exists("track_analysis", "feature_confidence"):
